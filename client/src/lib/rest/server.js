...
export default class Server {
	...
	static update_cache(server_id, data){
		if(Server.server_cache.has_state(server_id)){
			const server_data = Server.server_cache.get_state(server_id);
			for(const f in data)
	...
	static remove_cache(server_id){
		...
		const list_state = Server.server_list_cache.get_state(0);
		const server_list_idx = list_state.data.findIndex((x) => x.id === server_id);
