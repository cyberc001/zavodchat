...
void db_init(std::string conn_str)
{
	pqxx::connection conn{conn_str};
	{
		...
		tx.exec("CREATE TABLE IF NOT EXISTS emojis(emoji_id SERIAL PRIMARY KEY, server_id INTEGER REFERENCES servers ON DELETE CASCADE NOT NULL, name VARCHAR(64) NOT NULL, image VARCHAR(128) NOT NULL)");
