...
void db_init(std::string conn_str)
{
	pqxx::connection conn{conn_str};
	{
		...
		tx.exec("CREATE TABLE IF NOT EXISTS channel_notifications(channel_id INTEGER REFERENCES channels ON DELETE CASCADE NOT NULL, user_id INTEGER REFERENCES channels ON DELETE CASCADE NOT NULL, notification_count INTEGER NOT NULL DEFAULT 1, PRIMARY KEY(channel_id, user_id))");
