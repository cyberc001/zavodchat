...
Исходящий запрос дружбы был отклонён (в том числе при блокировании).
...
Пользователь удалил вас из друзей (в том числе при блокировании).
...
Если server_id отсутствует, то это личное сообщение (direct message).
...
В `data` включаются только изменённые состояния.

Состояния для `deaf` и `mute`:
* **0:** Не в муте/заглушении
* **1:** Самостоятельный мут/заглушение
* **2:** Мут/заглушение администратором
